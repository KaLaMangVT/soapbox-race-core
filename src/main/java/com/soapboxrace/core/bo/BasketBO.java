...
public class BasketBO {
    ...
    public CommerceResultStatus buyCarSlots(String productId, PersonaEntity personaEntity, TokenSessionEntity tokenSessionEntity) {
        int CarLimit = parameterBO.getCarLimit(tokenSessionEntity.getUserEntity());

        ProductEntity garageSlotProduct = productDao.findByProductId(productId);

        float price = productDao.findByProductId(productId).getPrice();
        if ((personaEntity.getnumCarSlots()) >= CarLimit) {
            return CommerceResultStatus.FAIL_INSUFFICIENT_FUNDS;
        }
        if (this.canPurchaseProduct(personaEntity, garageSlotProduct, price)) {

            this.performPersonaTransaction(personaEntity, garageSlotProduct, price);
            personaBo.setnumCarSlots(personaEntity);
